...
from ..team_utils import get_user_team
...
def _notify_match_ready(match: Match) -> None:
    ...
    text = (
        f"Матч готов.\n"
        f"Турнир: {match.tournament.title}\n"
        f"Раунд: {match.round}\n"
        f"Соперник: {match.team1.name} vs {match.team2.name}"
        f"{suffix}"
    )
...
def _notify_tournament_started(db: Session, tournament: Tournament) -> None:
    ...
    first_round_matches = (
        db.query(Match)
        .filter(Match.tournament_id == tournament.id, Match.round == 1)
...
def toggle_check_in(
    ...
    # Ищем именно регистрацию команды капитана в этом турнире
    tt = (
        db.query(TournamentTeam)
